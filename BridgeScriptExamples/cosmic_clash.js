...
function draw() {
  ...
  if (!gameOver) {
    // Player movement
    if (keyIsDown(65) && playerX > 0) { // 'A' key
      playerX -= playerSpeed;
    }
    if (keyIsDown(68) && playerX < screenWidth - playerWidth) { // 'D' key
      playerX += playerSpeed;
    }
    if (keyIsDown(87) && playerY > 0) { // 'W' key
      playerY -= playerSpeed;
    }
    if (keyIsDown(83) && playerY < screenHeight - playerHeight) { // 'S' key
      playerY += playerSpeed;
    }
...
function keyPressed() {
  if (key === ' ' && !gameOver) {
    if (tripleShot) {
      bullets.push({ x: playerX + playerWidth / 2, y: playerY, angle: 0 });
      bullets.push({ x: playerX + playerWidth / 2, y: playerY, angle: -PI / 4 });
      bullets.push({ x: playerX + playerWidth / 2, y: playerY, angle: PI / 4 });
    } else {
      bullets.push({ x: playerX + playerWidth / 2, y: playerY, angle: 0 });
    }
